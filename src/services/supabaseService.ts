  ...
  async directSyncAllLocalDataToSupabase(
    ...
    try {
      ...
      const toolRows = TOOLS.map((tool) => ({
        ...
        how_it_works: tool.howItWorks || null,
        example_scenario: tool.exampleScenario || null,
        faqs: tool.faqs || [],
        related_tool_ids: tool.relatedToolIds || [],
        disclaimer_type: tool.disclaimerType || null,
        is_active: true,
      }));
      ...
      const { error: blogErr } = await supabase.from('blogs').upsert(blogRows, { onConflict: 'slug' });
      if (blogErr) {
      ...
      return {
        success: true,
        message: `Successfully synchronized ${categoryRows.length} categories, ${toolRows.length} tools, and ${blogRows.length} blog guides to Supabase!`,
        counts: {
