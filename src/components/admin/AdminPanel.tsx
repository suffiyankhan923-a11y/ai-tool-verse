...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onNavigate }) => {
  ...
  const [showLoginConfigDrawer, setShowLoginConfigDrawer] = useState(false);
  ...
  // Supabase Auth Session Hydration on Mount
  useEffect(() => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!supabase) {
      setLoginError(
        'Supabase is not configured yet. Please expand "Supabase Project Settings" below and enter your Project URL and Anon Key.'
      );
      setShowLoginConfigDrawer(true);
      return;
    }
    ...
    try {
      ...
      if (error) {
        setLoginError(
          error.message ||
            'Invalid credentials. Please verify the email and password registered under Authentication > Users in your Supabase dashboard.'
        );
  ...
  // Handle Forgot / Reset Password via Supabase Auth
  const handleForgotPassword = async () => {
    if (!loginEmail.trim()) {
      setLoginError('Please enter your Supabase admin email in the email field first.');
      return;
    }

    const supabase = getSupabaseClient();
    if (!supabase) {
      setLoginError('Supabase is not configured. Please expand settings below.');
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        setResetSentMessage(`Password recovery link sent to ${loginEmail.trim()}. Check your inbox.`);
      }
    } catch (err: any) {
      setLoginError(err.message || 'Failed to dispatch password recovery email.');
    } finally {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md p-6 sm:p-8 rounded-3xl bg-[#0F172A] border border-[#D4AF37]/30 shadow-2xl shadow-black/90 space-y-6">
          {/* Header */}
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-[#94A3B8]">
              Sign in with your Supabase Authentication administrator account.
            </p>

            {/* Connection Status Badge */}
            <div className="pt-1 flex justify-center">
              {supabaseConfigSource !== 'none' ? (
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-[11px] font-mono">
                  ...
                  <span>Supabase Connected ({supabaseConfigSource.toUpperCase()})</span>
                </div>
              ) : (
                <button
                  type="button"
                  onClick={() => setShowLoginConfigDrawer(true)}
                  className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-[11px] font-mono hover:bg-amber-500/20 transition-colors"
                >
                  <AlertCircle className="w-3.5 h-3.5 text-amber-400" />
                  <span>Supabase Config Needed • Click to Set</span>
                </button>
              )}
            </div>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-3.5 py-2.5 rounded-xl bg-[#161E31] border border-[#D4AF37]/20 focus:border-[#D4AF37] text-white text-xs placeholder-[#475569] focus:outline-none"
                />
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-semibold text-[#E2E8F0]">
                  Admin Password
                </label>
                <button
                  type="button"
                  ...
                  className="text-[11px] text-[#D4AF37] hover:text-[#E5C158] hover:underline transition-colors"
                >
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-10 py-2.5 rounded-xl bg-[#161E31] border border-[#D4AF37]/20 focus:border-[#D4AF37] text-white text-xs placeholder-[#475569] focus:outline-none font-mono"
                />
                <button
                  type="button"
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-gradient-to-r from-[#D4AF37] to-[#B38F24] hover:from-[#E5C158] hover:to-[#C59F33] text-[#050810] font-bold text-xs shadow-lg shadow-[#D4AF37]/20 flex items-center justify-center gap-2 transition-all disabled:opacity-60 cursor-pointer"
            >
              {isLoggingIn ? (
                <>
          ...
          {/* Quick Supabase Connection Drawer Toggle */}
          <div className="pt-2 border-t border-[#D4AF37]/15">
            <button
              type="button"
              onClick={() => setShowLoginConfigDrawer(!showLoginConfigDrawer)}
              className="w-full py-2 px-3 rounded-xl bg-[#161E31]/60 hover:bg-[#161E31] text-[11px] text-[#94A3B8] hover:text-white flex items-center justify-between transition-colors border border-[#D4AF37]/10"
            >
              <span className="flex items-center gap-1.5">
                <Database className="w-3.5 h-3.5 text-[#D4AF37]" />
                <span>Supabase Connection Settings</span>
              </span>
              {showLoginConfigDrawer ? (
            ...
            {showLoginConfigDrawer && (
              <div className="mt-3 p-4 rounded-2xl bg-[#161E31] border border-[#D4AF37]/20 space-y-3">
                <p className="text-[11px] text-[#94A3B8]">
                  Specify your Supabase project credentials (from Project Settings &gt; API):
                </p>
                <div>
                  <label className="text-[10px] font-mono text-[#D4AF37] block mb-1">
                  ...
                  <input
                    ...
                    className="w-full px-2.5 py-1.5 rounded-lg bg-[#0F172A] border border-[#D4AF37]/20 text-white text-xs font-mono focus:outline-none focus:border-[#D4AF37]"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-mono text-[#D4AF37] block mb-1">
                ...
                <div className="flex items-center gap-2 pt-1">
                  <button
                    ...
                    className="flex-1 py-1.5 rounded-lg bg-[#D4AF37]/20 hover:bg-[#D4AF37]/30 text-[#D4AF37] text-[11px] font-bold border border-[#D4AF37]/40 transition-colors"
                  >
      ...
      <div className="p-4 sm:p-6 rounded-2xl bg-[#0F172A] border border-[#D4AF37]/30 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#94A3B8] mt-0.5">
              Logged in as <span className="text-[#D4AF37] font-semibold">{adminUser?.email || 'Administrator'}</span>
              {adminUser?.role && (
                <span className="ml-2 px-1.5 py-0.5 rounded bg-[#161E31] border border-[#D4AF37]/20 text-[10px] text-[#A3E635] font-mono">
                  {adminUser.role}
                </span>
              )}
            </p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 text-xs font-semibold flex items-center gap-1.5 border border-rose-500/30 transition-colors cursor-pointer"
          >
            <LogOut className="w-3.5 h-3.5" />
            <span>Logout ({adminUser?.email?.split('@')[0] || 'Admin'})</span>
          </button>
        </div>
      </div>
