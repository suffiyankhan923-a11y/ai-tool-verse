...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onNavigate }) => {
  ...
  // Supabase Live Connection Tester
  ...
  const handleTestSupabaseConnection = async () => {
    ...
    try {
      ...
      // Perform a ping check on Supabase REST endpoint
      ...
      if (response.ok || response.status === 200 || response.status === 404) {
        // Now test querying categories table if exists
        try {
          const catRes = await fetch(`${cleanUrl}/rest/v1/categories?select=count`, {
            method: 'GET',
            headers: {
              apikey: supabaseAnonKey.trim(),
              Authorization: `Bearer ${supabaseAnonKey.trim()}`,
          ...
          if (catRes.ok) {
            const countHeader = catRes.headers.get('content-range');
            setConnectionResult({
              success: true,
              message: `Successfully connected to Supabase! The "categories" table is active. (${countHeader || 'Ready'})`,
            });
          } else {
            setConnectionResult({
              success: true,
              message: `Connected to Supabase Project API successfully! Tables are ready to be created using the SQL below.`,
            });
          }
        } catch (e) {
          setConnectionResult({
            success: true,
            message: `Connected to Supabase Project API endpoint successfully!`,
          });
        }
      } else {
        setConnectionResult({
          success: false,
          message: `Supabase returned status code: ${response.status} (${response.statusText}). Check your URL and Key.`,
        });
      }
    } catch (err: any) {
      setConnectionResult({
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {activeTab === 'supabase' && (
        <div className="space-y-8 animate-in fade-in duration-200">
          ...
          {/* Optional: Live Supabase Connection Tester */}
          <div className="p-6 rounded-2xl bg-[#0F172A] border border-[#D4AF37]/20 space-y-4">
            <div className="flex items-center gap-2">
              <Zap className="w-5 h-5 text-[#D4AF37]" />
              <h3 className="text-base font-bold text-white">
                Live Supabase Connection & Ping Tester (Optional)
              </h3>
            </div>
            <p className="text-xs text-[#94A3B8]">
              Want to test if your Supabase project API is reachable from your browser? Enter your Project URL & Anon public key below:
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 rounded-xl bg-[#161E31] border border-[#D4AF37]/20 text-white font-mono text-xs placeholder-[#475569] focus:outline-none"
                />
              </div>
              <div>
                <label className="text-xs font-semibold text-[#E2E8F0] block mb-1">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-5 py-2 rounded-xl bg-[#D4AF37] hover:bg-[#E5C158] text-[#050810] font-bold text-xs flex items-center gap-2 disabled:opacity-50"
              >
                {testingConnection ? (
              ...
              {connectionResult && (
                <div
                  className={`text-xs font-semibold flex items-center gap-1.5 ${
                    connectionResult.success ? 'text-emerald-400' : 'text-rose-400'
                  }`}
                >
