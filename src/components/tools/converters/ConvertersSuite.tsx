...
export const TimeZoneConverter: React.FC = () => {
  ...
  const TIMEZONES = [
    ...
    { label: 'San Francisco / LA (PDT / PST)', zone: 'America/Los_Angeles' },
    { label: 'London / UTC (GMT / BST)', zone: 'Europe/London' },
  ...
  const getTimeInZone = (targetZone: string) => {
    try {
      const dt = new Date(`${baseDate}T${baseTime}:00`);
    ...
    <div className="space-y-6">
      <div className="p-5 md:p-6 rounded-2xl bg-white dark:bg-[#0c1322] border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            <label className="text-xs font-semibold block mb-1 text-slate-500">Date</label>
            <input
              type="date"
              value={baseDate}
          ...
          <div>
            <label className="text-xs font-semibold block mb-1 text-slate-500">Time</label>
            <input
              type="time"
              value={baseTime}
          ...
          <div>
            <label className="text-xs font-semibold block mb-1 text-slate-500">Base Timezone</label>
            <select
              value={sourceTz}
          ...
          <div
            ...
            className={`p-4 rounded-xl border transition-all ${
              tz.zone === sourceTz
                ...
                : 'bg-white dark:bg-[#0c1322] border-slate-200 dark:border-slate-800'
            }`}
          >
            <div className="flex justify-between items-center mb-1">
              <span className="text-xs font-semibold">{tz.label.split('(')[0]}</span>
              <span className="text-[10px] text-slate-400">
                {tz.label.includes('(') ? `(${tz.label.split('(')[1]}` : ''}
              </span>
            </div>
            <span className="text-lg font-bold font-mono block text-slate-900 dark:text-white">
...
export const CsvJsonConverter: React.FC = () => {
  ...
  const [inputContent, setInputContent] = useState(
    'id,name,email,role,status\n1,Alice Johnson,alice@company.com,Developer,Active\n2,Bob Smith,bob@company.com,Designer,Active\n3,Charlie Brown,charlie@company.com,Manager,Pending'
  );
      ...
      if (mode === 'csv-to-json') {
        ...
        const rows = lines.slice(1).map((line) => {
          ...
          headers.forEach((h, i) => {
            ...
            if (val.toLowerCase() === 'true') rowObj[h] = true;
            else if (val.toLowerCase() === 'false') rowObj[h] = false;
            else if (!isNaN(Number(val)) && val !== '') rowObj[h] = Number(val);
            else rowObj[h] = val;
          });
          return rowObj;
        });
        setOutputContent(JSON.stringify(rows, null, 2));
      } else {
        ...
        setOutputContent(csvLines.join('\n'));
      }
    } catch (e: any) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          type="button"
          onClick={convertData}
          className="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs shadow-sm flex items-center gap-1.5"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="p-5 md:p-6 rounded-2xl bg-white dark:bg-[#0c1322] border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-3">
          <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block">
            {mode === 'csv-to-json' ? 'Input CSV Text' : 'Input JSON Array'}
          </label>
          <textarea
            rows={12}
            value={inputContent}
            onChange={(e) => setInputContent(e.target.value)}
            className="w-full p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 font-mono text-xs text-slate-900 dark:text-slate-100 leading-relaxed"
          />
        </div>

        <div className="p-5 md:p-6 rounded-2xl bg-white dark:bg-[#0c1322] border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-3 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block">
                {mode === 'csv-to-json' ? 'Output JSON Array' : 'Output CSV Text'}
              </label>
              <div className="flex gap-2">
                <button
                  type="button"
