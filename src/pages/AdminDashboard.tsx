...
export const AdminDashboard: React.FC = () => {
  ...
  // Security & Supabase Settings States
  ...
  const [sbResult, setSbResult] = useState<{ success: boolean; msg: string } | null>(null);
  ...
  const [passFeedback, setPassFeedback] = useState<{ success: boolean; msg: string } | null>(null);
  ...
  const handleUpdatePassword = (e: React.FormEvent) => {
    ...
    if (newPass !== confirmPass) {
      setPassFeedback({ success: false, msg: 'New password and confirmation do not match.' });
      return;
    }

    const result = updateBuiltInPassword(oldPass, newPass);
    if (result.success) {
      setPassFeedback({ success: true, msg: 'Super Admin password updated successfully!' });
      setOldPass('');
      setNewPass('');
      setConfirmPass('');
    } else {
      setPassFeedback({ success: false, msg: result.error || 'Failed to update password.' });
    }
  };

  const handleSaveSbSettings = async (e: React.FormEvent) => {
    e.preventDefault();
    setSbTesting(true);
    setSbResult(null);

    const res = await updateSupabaseSettings(sbUrlInput, sbKeyInput);
    setSbTesting(false);
    setSbResult({
      success: res.success,
      msg: res.message
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[#FAF7F2] dark:bg-[#22252E] border border-[#EAE2D5] dark:border-[#2C303B] text-xs">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-[#756E65] dark:text-[#9E9B96]">
                {user?.provider === 'supabase' ? 'Supabase Auth' : 'Built-in Super Admin'}
              </span>
            </div>
          </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-[#EAE2D5] dark:border-[#2C303B] pb-3">
        {[
          ...
          {
            id: 'settings',
            label: isSupabaseConfigured ? 'Supabase Auth & Security' : 'Admin Security & Auth',
            icon: ShieldCheck
          },
        ].map((tab) => {
          const Icon = tab.icon;
      ...
      {/* TAB 6: SECURITY & SUPABASE AUTH SETTINGS */}
      {activeTab === 'settings' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Card 1: Supabase Auth & Project Configuration */}
          <div className="bg-white dark:bg-[#181A20] border border-[#EAE2D5] dark:border-[#2C303B] rounded-3xl p-6 sm:p-7 shadow-xs space-y-5">
            <div className="flex items-center justify-between pb-3 border-b border-[#EAE2D5]/80 dark:border-[#2C303B]/80">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#756E65] dark:text-[#9E9B96]">
                    Connect your project to manage admin logins from Supabase Auth &gt; Users
                  </p>
                </div>
              </div>
              ...
              >
                {isSupabaseConfigured ? 'Supabase Active' : 'Built-in Mode'}
              </span>
            </div>

            {/* Supabase credentials form */}
            <form onSubmit={handleSaveSbSettings} className="space-y-3.5 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-[#EAE2D5] dark:border-[#2C303B] bg-[#FAF7F2] dark:bg-[#22252E] text-xs text-[#1F1B18] dark:text-[#F7F5F0] focus:outline-none focus:border-[#B5824C]"
                  required
                />
              </div>
              ...
              {sbResult && (
                ...
                >
                  {sbResult.success ? <CheckCircle2 className="w-4 h-4 mt-0.5 shrink-0" /> : <AlertTriangle className="w-4 h-4 mt-0.5 shrink-0" />}
                  <span>{sbResult.msg}</span>
                </div>
              )}
              ...
              <div className="flex items-center gap-2 pt-1">
                <button
                  type="submit"
                  disabled={sbTesting}
                  className="flex-1 inline-flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-[#B5824C] hover:bg-[#9E6F3B] disabled:opacity-50 text-white font-semibold text-xs shadow-xs transition-colors cursor-pointer"
                >
                  {sbTesting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
                ...
                {isSupabaseConfigured && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setSbUrlInput('');
                      setSbKeyInput('');
                      setSbResult({ success: true, msg: 'Supabase disconnected. Switched back to Built-in Super Admin auth.' });
                    }}
                    className="px-3 py-2.5 rounded-xl bg-white dark:bg-[#181A20] border border-red-200 dark:border-red-900/40 text-red-600 text-xs font-semibold hover:bg-red-50 cursor-pointer"
                  >
                    Disconnect
                  </button>
                )}
            ...
            <div className="p-5 rounded-2xl bg-[#FAF7F2] dark:bg-[#22252E] border border-[#EAE2D5]/80 dark:border-[#2C303B]/80 text-xs space-y-4">
              <div className="flex items-center justify-between">
                <div className="font-bold text-[#1F1B18] dark:text-[#F7F5F0] flex items-center gap-1.5">
                  ...
                  <span>Supabase Database &amp; Tables Setup</span>
                </div>
                <button
                  type="button"
              ...
              <p className="text-[11px] text-[#756E65] dark:text-[#9E9B96] leading-relaxed">
                Run our turnkey SQL schema in your <strong>Supabase SQL Editor</strong> to create all tables (<code>categories</code>, <code>tools</code>, <code>blogs</code>, <code>faqs</code>, <code>contact_messages</code>) with optimized Row Level Security (RLS) policies.
              </p>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                ...
                <button
                  ...
                  disabled={pushingToSupabase || !isSupabaseConfigured}
                  ...
                  title={!isSupabaseConfigured ? 'Please configure and save your Supabase URL & Anon Key first' : 'Push local categories, tools, and blogs to Supabase tables'}
                >
                  {pushingToSupabase ? (
            ...
            <div className="p-4 rounded-2xl bg-[#FAF7F2] dark:bg-[#22252E] border border-[#EAE2D5]/80 dark:border-[#2C303B]/80 text-xs text-[#756E65] dark:text-[#9E9B96] space-y-2">
              ...
              <ol className="list-decimal list-inside space-y-1.5 pl-1 text-[11px] leading-relaxed">
                <li>
                  <strong>Step 1 (API Keys):</strong> In your <em>Supabase Dashboard &gt; Project Settings &gt; API</em>, copy your <strong>Project URL</strong> and <strong>anon key</strong>, paste them above, and click <strong>Test &amp; Save</strong>.
                </li>
                <li>
                  <strong>Step 2 (Tables):</strong> Click <strong>Copy SQL Schema</strong>, go to your <em>Supabase Dashboard &gt; SQL Editor</em>, paste the script, and click <strong>Run</strong>. Then click <strong>Push All Content to Supabase</strong> above!
                </li>
                <li>
                  <strong>Step 3 (Admin Users):</strong> Go to <em>Supabase Dashboard &gt; Authentication &gt; Users &gt; Add user</em> to create your administrator logins.
                </li>
              </ol>
            </div>
          </div>

          {/* Card 2: Super Admin Security & Session */}
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-[#181A20] border border-[#EAE2D5] dark:border-[#2C303B] rounded-3xl p-6 sm:p-7 shadow-xs space-y-4">
              <div className="flex items-center gap-2.5 pb-3 border-b border-[#EAE2D5]/80 dark:border-[#2C303B]/80">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#756E65] dark:text-[#9E9B96]">
                    Active login token &amp; privilege verification
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className="p-3 rounded-xl bg-[#FAF7F2] dark:bg-[#22252E] border border-[#EAE2D5]/80 dark:border-[#2C303B]/80">
                  ...
                  <span className="font-bold text-[#B5824C] dark:text-[#DFB267] block">
                    {user?.provider === 'supabase' ? 'Supabase Auth' : 'Built-in Admin'}
                  </span>
                </div>
                <div className="p-3 rounded-xl bg-[#FAF7F2] dark:bg-[#22252E] border border-[#EAE2D5]/80 dark:border-[#2C303B]/80">
                  <span className="text-[10px] text-[#756E65] dark:text-[#9E9B96] block uppercase tracking-wider font-semibold">Role Level</span>
                  <span className="font-bold text-[#1F1B18] dark:text-[#F7F5F0] block">{user?.role || 'Super Admin'}</span>
                </div>
                <div className="p-3 rounded-xl bg-[#FAF7F2] dark:bg-[#22252E] border border-[#EAE2D5]/80 dark:border-[#2C303B]/80">
            ...
            {/* Change Built-in Super Admin Password */}
            <div className="bg-white dark:bg-[#181A20] border border-[#EAE2D5] dark:border-[#2C303B] rounded-3xl p-6 sm:p-7 shadow-xs space-y-4">
              <div className="flex items-center gap-2.5 pb-3 border-b border-[#EAE2D5]/80 dark:border-[#2C303B]/80">
                ...
                <div>
                  <h3 className="font-bold text-sm text-[#1F1B18] dark:text-[#F7F5F0] font-['Outfit',sans-serif]">
                    Change Built-in Master Password
                  </h3>
                  <p className="text-[11px] text-[#756E65] dark:text-[#9E9B96]">
                    Update the fallback credentials for <code className="font-mono text-[#B5824C]">admin@toolverse.com</code>
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleUpdatePassword} className="space-y-3 text-xs">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2 rounded-xl border border-[#EAE2D5] dark:border-[#2C303B] bg-[#FAF7F2] dark:bg-[#22252E] text-xs text-[#1F1B18] dark:text-[#F7F5F0] focus:outline-none focus:border-[#B5824C]"
                    required
                  />
                </div>
                ...
                {passFeedback && (
                  ...
                  >
                    {passFeedback.success ? <CheckCircle2 className="w-4 h-4 shrink-0" /> : <AlertTriangle className="w-4 h-4 shrink-0" />}
                ...
                <button
                  type="submit"
                  className="w-full inline-flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-[#FAF7F2] dark:bg-[#22252E] border border-[#EAE2D5] dark:border-[#2C303B] hover:border-[#B5824C] text-[#1F1B18] dark:text-[#F7F5F0] font-bold text-xs shadow-xs transition-colors cursor-pointer"
                >
                  <Key className="w-3.5 h-3.5 text-[#B5824C] dark:text-[#DFB267]" />
