...
3. Readability & Tone Assessment Score (e.g. 95/100).`;
  ...
  app.get("/api/admin/export", (req, res) => {
    try {
      const categories = queryAll("SELECT * FROM categories ORDER BY id ASC");
      const toolsRaw = queryAll<any>("SELECT * FROM tools ORDER BY id ASC");
      const blogs = queryAll("SELECT * FROM blogs ORDER BY id ASC");
      const faqs = queryAll("SELECT * FROM faqs ORDER BY id ASC");

      const tools = toolsRaw.map((t) => {
        let parsedFaq = [];
        try {
          parsedFaq = typeof t.faq === "string" ? JSON.parse(t.faq || "[]") : t.faq;
        } catch {
          parsedFaq = [];
        }
        return {
      ...
      res.setHeader("Content-Disposition", 'attachment; filename="toolverse-complete-dataset.json"');
